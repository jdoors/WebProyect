    ...
    <div className="py-12 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="lg:text-center">
          <p className="font-cormorant mt-2 text-4xl leading-10 font-bold tracking-tight text-gray-900 ">
          ...
          <dl className="space-y-10 md:space-y-0 md:grid md:grid-cols-4 md:gap-x-8 md:gap-y-10">
            {caracteristicas.map((caracteristicas) => (
              <div key={caracteristicas.name} className="relative">
                <dt>
                  <div className="absolute flex items-center justify-center h-10 w-10">{caracteristicas.icon}</div>

                  <p className="font-poppins ml-16 text-lg leading-6 font-bold text-gray-900">{caracteristicas.name}</p>
                </dt>
                <dd className="font-poppins mt-2 ml-16 text-base text-gray-500">{caracteristicas.description}</dd>
              </div>
            ))}
          </dl>
        </div>
